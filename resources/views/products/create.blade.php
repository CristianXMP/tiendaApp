...
@section('content-card')
    <h5 class="card-title text-form">Nuevo Producto</h5>
    <hr>
    <div class="container">
        <form class="row" action="{{ route('products.store') }}" method="post">
            <div class="col-6 col-md-6 col-sm-12">
                ...
                <div class="form-group">
                    ...
                    <textarea rows="5   " columns="4" class="form-control" name="observation"
            ...
            <div class="col-6 col-md-6 col-sm-12">
                <div class="form-group">
                    ...
                    <select name="trademark" class="custom-select">
                        ...
                        @foreach ($trademarks as $trademark)
                            <option  value="{{ $trademark->id}}" @if (old('trademark') == $trademark->id) selected @endif>{{ $trademark->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" placeholder="Ejemplo: 60" name="stock"  value="{{ old('stock') }}" />

                    <p class="text-danger text-center"> {{ $errors->first('stock') }}</p>
                </div>
                <div class="form-group">
                    <label for="price">Precio</label>
                    <input type="number" class="form-control" placeholder="Ejemplo: 30000" name="price"  value="{{ old('price') }}"/>

                     <p class="text-danger text-center">{{ $errors->first('price') }}</p>
                </div>
                <div class="form-group">
                    <label for="shipping_date">Fecha de embarque</label>
                    <input type="date" class="form-control" placeholder="00/00/0000" name="shipping_date"  value="{{ old('shipping_date') }}"/>
